...
/// `on_glance` with the current glance lines, the raw inbox facts, and the
/// desktop notices just fired (so the app can mirror them into the Log). Owns
/// all threading + the runtime, so callers stay synchronous and UI-agnostic.
/// No-ops (the thread exits) if a runtime can't be built.
pub fn watch(on_glance: impl Fn(Vec<String>, Vec<inbox::InboxItem>, Vec<Notice>) + Send + 'static) {
    std::thread::spawn(move || {
        ...
        rt.block_on(async move {
            loop {
                ...
                on_glance(result.lines, result.inbox, result.fresh.clone());
