...
- **Version + About.** The three crate `Cargo.toml` files share a calendar
  `YYYY.M.P` version. Settings → About and the docs header read it from
  `crates/repoharbor/Cargo.toml`. When shipping a user-facing change, bump all
  three crate versions together. Public identity (author, email, phone, repo)
  lives in `crates/repoharbor/src/identity.rs`, `COPYRIGHT.md`, and
  `docs/guide/copyright.md`. Keep documentation in English. Do not commit
  secrets, client project names, agent transcripts, or machine-local config.
